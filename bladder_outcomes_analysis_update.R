...
bladder_surv_points <- ggplot(data = bladder_data_cleaned, aes(x = SURV, y = VARIANT)) + geom_point(aes(shape = factor(CENS)), fill = 'white') + scale_shape_manual(values = c(19,21)) + theme_bw() + ylab('') + xlab('Time (months)')
